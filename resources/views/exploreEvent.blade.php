...
?>
@if ($current <= $date_exspire) @if (Auth::id() !=$event->user_id)
        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-sm-4" data-toggle="modal" data-target="#eventDetail{{$event->id}}">
              ...
              @if ($event->joins->count("user_id") <= 1) <p> <strong class="text-warning ">{{$event->joins->count("user_id")}}</strong> member going</p>
                @endif
  ...
  <script>
    ...
    function joinButton() {
      var eventJoin = {
        !!json_encode($joinEvent, JSON_HEX_TAG) !!
      }
      var user_id = {
        !!json_encode(Auth::id(), JSON_HEX_TAG) !!
      }
